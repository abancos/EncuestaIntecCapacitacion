				...
				$("#survey_container").wizard({
					transitions: {
						branchtype: function( $step, action ) {
						    var branch = $step.find("[name=CorporativoRes]:checked").val();

                            

							if (branch == 'satisfied') {
							    var branch = $step.find("[name=PymesRes]:checked").val();

							    if (!branch) {
							        var branch = 'satisfied';
							    }
								
							}

							if (!branch) {
							    alert("Por favor completar el B.1");
							}
							return branch;
						}
					}
